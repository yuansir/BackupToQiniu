...
tar_name = strftime("%Y-%m-%d", gmtime())
...
def upToQiniu():
    upload_file = os.getcwd() + '/' + tar_file
    if os.path.exists(upload_file) != True:
        log('Error', 'tar fail')
        sys.exit()

    q = qiniu.Auth(auth.ACCESS_KEY, auth.SECRET_KEY)
    uptoken = q.upload_token(common.BUCKET_NAME,tar_file)
    ret, info = qiniu.put_file(uptoken, tar_file, upload_file)
